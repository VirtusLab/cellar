...
class ProjectAwareIntegrationTest extends CatsEffectSuite:
  ...
  test("MillBuildTool: rejects missing --module"):
    build.MillBuildTool(Path("."), Config.bootstrap.mill)
  ...
  test("SbtBuildTool: rejects missing --module"):
    build.SbtBuildTool(Path("."), Config.bootstrap.sbt).extractClasspath(None)
      .attempt.map { result =>
        assert(result.isLeft)
  ...
  test("E2E scala-cli: get resolves project symbol"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Main.scala").toNioPath,
        ...
        handlers.ProjectGetHandler.run("example.MyClass", module = None, Config.bootstrap, cwd = Some(dir)).map { code =>
  ...
  test("E2E scala-cli: get resolves symbol from dependency"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Main.scala").toNioPath,
        ...
        handlers.ProjectGetHandler.run("cats.Monad", module = None, Config.bootstrap, cwd = Some(dir)).map { code =>
  ...
  test("E2E scala-cli: list lists members of project class"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Main.scala").toNioPath,
        ...
        handlers.ProjectListHandler.run("example.MyClass", module = None, limit = 50, Config.bootstrap, cwd = Some(dir)).map { code =>
          assertEquals(code, ExitCode.Success)
  ...
  test("E2E scala-cli: search finds symbols across project"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Main.scala").toNioPath,
        ...
        handlers.ProjectSearchHandler.run("UniqueTestClassName123", module = None, limit = 50, Config.bootstrap, cwd = Some(dir)).map { code =>
          assertEquals(code, ExitCode.Success)
  ...
  test("E2E scala-cli: --module produces error"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Main.scala").toNioPath,
        ...
        handlers.ProjectGetHandler.run("example.Foo", module = Some("bar"), Config.bootstrap, cwd = Some(dir)).map { code =>
  ...
  test("E2E scala-cli: compilation failure surfaces build tool error"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("Bad.scala").toNioPath,
        ...
        handlers.ProjectGetHandler.run("example.Bad", module = None, Config.bootstrap, cwd = Some(dir)).map { code =>
          assertEquals(code, ExitCode.Error)
  ...
  test("E2E Mill: get resolves project symbol"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking {
        ...
        handlers.ProjectGetHandler.run("example.MillClass", module = Some("app"), cwd = Some(dir), config = Config.bootstrap.copy(mill = MillConfig(millBinary))).map { code =>
  ...
  test("E2E Mill: --module required"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("build.mill").toNioPath, "")) >>
        handlers.ProjectGetHandler.run("example.Foo", module = None, cwd = Some(dir), config = Config.bootstrap.copy(mill = MillConfig(millBinary))).map { code =>
          assertEquals(code, ExitCode.Error)
  ...
  test("E2E sbt: get resolves project symbol"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking {
        ...
        handlers.ProjectGetHandler.run("example.SbtClass", module = Some("cellar-test"), Config.bootstrap, cwd = Some(dir)).map { code =>
          assertEquals(code, ExitCode.Success, s"Stderr: ${console.errBuf}")
  ...
  test("E2E sbt: --module required"):
    ...
    withTempDir { dir =>
      ...
      IO.blocking(Files.writeString(dir.resolve("build.sbt").toNioPath, "")) >>
        handlers.ProjectGetHandler.run("example.Foo", module = None, Config.bootstrap, cwd = Some(dir)).map { code =>
          assertEquals(code, ExitCode.Error)
